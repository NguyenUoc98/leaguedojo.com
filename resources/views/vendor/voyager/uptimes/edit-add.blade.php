                                    ...
                                    @else
                                        ...
                                        <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $field['grid'] ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-xs-5">
                                                    <label class="control-label">Đóng cửa</label>
                                                </div>
                                            </div>
                                            @endif
                                            ...
                                            @elseif ($row->field == 'uptimes')
                                            @php
                                                $uptimes = json_decode($dataTypeContent->{$row->field});
                                            @endphp
                                            <div class="add-field">
                                                <div class="clonedInput" id="removeId1">
                                                    <div class="row">
                                                        <div class="col-xs-5">
                                                            <input required type="time" class="form-control" name="start_at[]" id="start_at" value="{{ $uptimes[0][0] }}">
                                                        </div>
                                                        <div class="col-xs-5">
                                                            <input required type="time" class="form-control" name="end_at[]" id="end_at" value="{{ $uptimes[0][1] }}">
                                                        </div>
                                                    </div>
                                                </div>

                                                @if(!is_null($uptimes) && count($uptimes) > 1)
                                                @foreach($uptimes as $index=>$uptime)
                                                @if($index > 0)
                                                @include('voyager::uptimes.dynamic-fields', 
                                                        [
                                                            'id' => $index + 1,
                                                            'varId' => 'removeId' . ($index + 1),
                                                            'start_at' => $uptime[0],
                                                            'end_at' => $uptime[1]
                                                        ])
                                                @endif
...
<script>
    ...
    function createClone() {
        axios.post("{{ route('uptimes.clone-fields') }}", {
            divCount: ++divCount,
        })
        .then(response => {
