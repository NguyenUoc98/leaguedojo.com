...
@section('page_title', 'Thông tin '.$dataType->getTranslatedAttribute('display_name_plural'))
    ...
    <div class="container-fluid">
        ...
        @can('edit', app($dataType->model_name))
            @if(isset($dataType->order_column) && isset($dataType->order_display_column))
                <a href="{{ route('voyager.'.$dataType->slug.'.order') }}" class="btn btn-dark btn-add-new">
        ...
        @can('delete', app($dataType->model_name))
            @if($usesSoftDeletes)
                <input type="checkbox" @if ($showSoftDeleted) checked @endif 
                    id="show_soft_deletes" data-toggle="toggle" data-on="{{ __('voyager::bread.soft_deletes_off')}}" data-off="{{ __('voyager::bread.soft_deletes_on')}}">
            @endif
        @endcan
        ...
        @include('voyager::bread.partials.export', ['dataType' => $dataType])
    </div>
@stop
...
@section('content')
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'relationship')
                                                    @include('voyager::formfields.relationship', ['view' => 'browse','options' => $row->details, 'display_name' => $row->getTranslatedAttribute('display_name')])
                                                ...
                                                @elseif($row->type == 'date' || $row->type == 'timestamp' || $row->type == 'time' || $row->type == 'month')
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    <span style="line-height: 1.5;">
                                                        <span class="label label-dark">{{ $keyword }}</span><br>
                                                    </span>
                                                    @endforeach
                                                    @else
                                        ...
                                        <td>
                                            @if($data->isOnline())
                                            <span style="color: #52b757"> ● </span> Online
                                            @else
                                            <span style="color: #f44336"> ● </span> Offline
                                            @endif
                                        </td>
                                        <td class="no-sort no-click text-center" id="bread-actions" style="max-width: 225px;">
                                            @php
                                            $actions = [];
    ...
    @if(Auth::user()->hasPermission('confirm_' . $dataType->name))
    <!-- Single reject modal -->
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.export .dropdown-menu').on('click', function(event) {
